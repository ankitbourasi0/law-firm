...
const Categories = () => {
  ...
  return (
    <div className="bg-yellow-200 border border-neutral-800  text-neutral-800 shadow-lg rounded-lg px-6 py-6 mb-6 pb-4">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-4 font-semibold text-neutral-700 ">
        ...
        <Link key={category.slug} href={`/category/${category.slug}`}>
          <p className="cursor-pointer shadow-md px-4 py-2 bg-orange-300 rounded-full ">
            {category.name}
          </p>
        </Link>
      ))}</div>
